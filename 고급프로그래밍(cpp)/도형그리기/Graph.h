...
struct Box : Shape {
	Box(Point p, int rr, int ww, int hh) :r{ rr }, w{ ww }, h{ hh } {
		add(Point{ p.x, p.y });
	}
...
struct Regular_polygon : Shape {
	Regular_polygon(Point p, int nn, int rr) : n{ nn }, r{ rr } {
		add(p);
	}
	void draw_lines() const;
	Point center() const { return { point(0).x,point(0).y }; }
	double side_length() const { return 2*(r * sin((360 / n) / 2 * 3.14)); }
private:
	int n;
	int r;
};
...
struct Star : Shape {
	Star(Point p, int nn, int rr1, int rr2) :n{ nn }, r1{ rr1 }, r2{ rr2 } {
		add(Point{ p.x,p.y });
	}

	void draw_lines() const;
	int radius_S() const { return r1; }
	int radius_L() const { return r2; }
