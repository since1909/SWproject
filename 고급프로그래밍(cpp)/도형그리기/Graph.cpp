...
void Regular_polygon::draw_lines() const
{
	if (color().visibility()) {
		...
		int angle = 360 / n;
		double rad = 3.14 / 180;
		for (int i = angle; i <= 360; i = i + angle)
		{
			Point tmp1;
			Point tmp2;
			tmp1.x = point(0).x + (r*sin(i*rad));
			tmp1.y = point(0).y - (r*cos(i*rad));
			tmp2.x = point(0).x + (r*sin((i + angle)*rad));
			tmp2.y = point(0).y - (r*cos((i + angle)*rad));
			fl_line(tmp1.x, tmp1.y, tmp2.x, tmp2.y);
		}
	}
}
...
void Star::draw_lines() const
{
	if (color().visibility()) {
		...
		for (int i = angle; i <= 360; i = i + angle)
		{
			...
			tmp1.x = point(0).x + (r2*sin(i*rad));
			tmp1.y = point(0).y - (r2*cos(i*rad));
			tmp2.x = point(0).x + (r1*sin((i + half_angle)*rad));
			tmp2.y = point(0).y - (r1*cos((i + half_angle)*rad));
			tmp3.x = point(0).x + (r2*sin((i + angle)*rad));
			tmp3.y = point(0).y - (r2*cos((i + angle)*rad));
			
			fl_line(tmp1.x, tmp1.y, tmp2.x, tmp2.y);
			fl_line(tmp2.x, tmp2.y, tmp3.x, tmp3.y);
